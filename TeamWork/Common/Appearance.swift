...
struct Appearance {
    ...
    static private func setUpTabBarAppearance() {
        let tabBar = UITabBar.appearance(whenContainedInInstancesOf: [AppTabBarController.self])
        tabBar.isTranslucent = false
    }

    static private func setUpNavigationAppearance() {
        let externalNavBar = UINavigationBar.appearance(whenContainedInInstancesOf: [ExternalNavigationController.self])
        let internalNavBar = UINavigationBar.appearance(whenContainedInInstancesOf: [InternalNavigationController.self])
        let transparentNavBar
            = UINavigationBar.appearance(whenContainedInInstancesOf: [TransparentNavigationController.self])

        externalNavBar.isTranslucent = false
        externalNavBar.prefersLargeTitles = true

        internalNavBar.isTranslucent = false
        internalNavBar.prefersLargeTitles = true

        transparentNavBar.barTintColor = .clear
